...
# [SYSTEM CONFIG]
st.set_page_config(page_title="D-Fi Vault v11.7", page_icon="🏛️", layout="wide")
...
st.markdown("""
    ...
    .stAlert { display: none; } /* 혹시 모를 경고창 강제 숨김 */
...
# ==========================================
# 🧠 [CORE LOGIC] 문맥 반영 해석 엔진 (Context Engine)
# ==========================================
def analyze_dream_engine(symbol, dynamics):
    # 1. 키워드 감지 (카테고리 확장)
    keywords = {
        "옷": "persona", "의복": "persona", "체육복": "persona", "유니폼": "persona", 
        "가면": "persona", "화장": "persona", "모자": "persona", "신발": "persona",
        
        "쫓김": "shadow", "도망": "shadow", "괴물": "shadow", "귀신": "shadow", "공격": "shadow", "살인마": "shadow",
        "돈": "wealth", "황금": "wealth", "보석": "wealth", "부자": "wealth", "주식": "wealth", "동전": "wealth",
        "집": "self", "방": "self", "건물": "self", "이사": "self", "청소": "self",
        "물": "unconscious", "바다": "unconscious", "강": "unconscious", "수영": "unconscious", "홍수": "unconscious",
    ...
    # 2. 해석 템플릿 (사용자의 입력값 symbol, dynamics를 문장에 직접 삽입)
    # {s} = symbol(상징), {d} = dynamics(역학)
    interpretations = {
        "persona": { # 옷, 체육복 등
            "jung": f"'{symbol}'은(는) 당신이 사회에서 보여주는 역할, 즉 '페르소나(Persona)'를 상징합니다. 당신이 '{dynamics}'라고 느낀 것은, 현재의 사회적 역할에 변화가 필요함을 무의식이 알리는 신호입니다.",
            "johnson": f"우리는 종종 맞지 않는 옷을 입고 살아갑니다. '{dynamics}'라는 역학은 당신의 내면과 겉모습(역할) 사이의 조율이 필요함을 암시합니다. 겉치레보다는 본질에 집중하십시오.",
            "ko": f"꿈속의 '{symbol}'은(는) 남들에게 보여주고 싶은 당신의 모습입니다. 하지만 '{dynamics}'의 상황은 이제 그 낡은 이미지를 벗어던지고 더 진실된 나를 드러내도 안전하다는 메시지입니다.",
            "ritual": f"오늘 하루, 평소에 입지 않던 스타일의 옷을 입거나 '{symbol}'과 관련된 물건을 정리하기"
        },
        "shadow": { # 쫓김, 괴물
            "jung": f"'{symbol}'은(는) 당신의 '그림자(Shadow)'입니다. 당신이 '{dynamics}'의 반응을 보인 것은, 내면의 억눌린 에너지가 통합을 요구하며 강렬하게 다가오고 있다는 뜻입니다.",
            "johnson": f"도망치지 마십시오. '{dynamics}'의 상황은 공포가 아니라 초대입니다. 이 에너지는 당신을 해치려는 것이 아니라, 당신의 삶에 활력을 주기 위해 찾아왔습니다.",
            "ko": f"그 추격자는 바로 '당신 자신'의 일부입니다. 당신이 '{dynamics}'하며 거부했던 그 힘을 다시 내 안으로 받아들일 때, 당신은 더 온전해집니다.",
            "ritual": f"'{symbol}'에게 이름을 붙여주고, '너는 나의 힘이다'라고 세 번 말하기"
        },
        "wealth": { # 돈, 황금
            "jung": f"'{symbol}'은(는) 단순한 물질이 아니라, 당신 내면의 '자기(Self)'가 가진 고귀한 가치를 상징합니다. '{dynamics}'의 상황은 당신의 영적 풍요가 현실화될 준비가 되었음을 의미합니다.",
            "johnson": f"이 풍요로움을 죄책감 없이 받아들이십시오. '{dynamics}'의 흐름은 내면의 창조적 에너지가 밖으로 흘러나와야 함을 보여줍니다.",
            "ko": f"당신은 이미 충분합니다. '{symbol}'을(를) 통해 보여준 무의식은, 당신이 가진 잠재력이 '{dynamics}'의 방식으로 세상에 기여할 수 있음을 암시합니다.",
            "ritual": f"지갑이나 통장을 손에 쥐고 '나는 이 풍요를 감당할 그릇이다'라고 선언하기"
        },
        "self": { # 집, 방
            "jung": f"'{symbol}'은(는) 당신의 인격 구조 그 자체입니다. 당신이 '{dynamics}'라고 묘사한 것은, 당신의 의식이 새로운 영역으로 확장되고 있거나 재구조화되고 있음을 뜻합니다.",
            "johnson": f"내면의 공간을 정비하십시오. '{dynamics}'의 느낌을 잘 살피십시오. 낡은 관념은 버리고 새로운 에너지가 들어올 공간을 마련해야 합니다.",
            "ko": f"이 공간은 당신의 마음입니다. '{symbol}'에서 느꼈던 '{dynamics}'의 감정을 현실의 내 방이나 공간에 적용해보세요. 공간의 변화가 마음의 변화를 부릅니다.",
            "ritual": f"내 방의 물건 중 하나를 버리거나 위치를 바꾸어 '{dynamics}'의 에너지 만들기"
        },
        "general": { # 일반
            "jung": f"'{symbol}'이라는 상징은 당신 무의식이 보낸 특별한 암호입니다. 특히 당신이 '{dynamics}'라고 느낀 부분에 주목하십시오. 그곳에 당신의 성장을 위한 열쇠가 숨어 있습니다.",
            "johnson": f"이 꿈을 머리로 분석하려 하지 말고, '{dynamics}'의 에너지 그 자체를 느껴보십시오. '{symbol}'은(는) 당신의 삶을 역동적으로 바꿀 촉매제입니다.",
            "ko": f"꿈에 나온 모든 것은 결국 당신의 모습입니다. 당신이 '{symbol}'이(가) 되어 '{dynamics}'의 상황을 겪는다고 상상해보세요. 그것이 당신에게 어떤 말을 건넵니까?",
            "ritual": f"'{symbol}'의 이미지를 간단히 그리거나, 그 단어를 종이에 적어 오늘 하루 주머니에 넣고 다니기"
        }
    }
    
    # 매칭된 카테고리가 있으면 반환, 없으면 general 반환 (단, fallback 로직 강화)
    result = interpretations.get(detected_type, interpretations["general"])
    return result

# ==========================================
# 🚪 GATES (v11.4 유지 - 생략 없이 포함)
# ==========================================
...
"{result['ko']}"
"""
            # 🔴 [경고창 해결] key와 value 동시 사용 문제 해결을 위해 session_state 직접 수정
    ...
    # 🔴 [핵심 수정] value 인자를 제거하고, default 값을 key로 관리
